...
spi_status_t query_ack(SPI_HandleTypeDef *hspi) {
    ...
    return spi_transaction(hspi, QUERY_ACKNOWLEDGE, NULL, 0, data_out, &data_length_out, 1, SPI_TIMEOUT_MS);
}
...
spi_status_t query_echo(SPI_HandleTypeDef *hspi) {
    ...
    spi_status_t status =
        spi_transaction(hspi, QUERY_ECHO, send_data, data_sending_len, data_out, &data_length_out, 67, SPI_TIMEOUT_MS);
...
spi_status_t query_getRTD(SPI_HandleTypeDef *hspi) {
    ...
    spi_status_t status =
        spi_transaction(hspi, QUERY_ECHO, NULL, 0, data_out, &data_length_out, sizeof(data_out), SPI_TIMEOUT_MS);
